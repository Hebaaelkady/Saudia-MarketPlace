...
;(function ($) {
    ...
    $.fn.treeSelect = function (options) {
        ...
        this.init = function (self) {
            ...
            //reset
            $(document).off('click', '#trsel-list .treeItem');
            // on click items
            $(document).on('click', '#trsel-list .treeItem', function () {
                // hast child
        ...
        this.showTree = function (list, cb) {
            var content = '<div class="container-fuild">';
            // clear li list
            $("#trsel-list .treeItem").remove();
            // back button handle
            // has parent need back button
            if ($.navigatex.length !== 0) {
                // if navigation is empity not need back button
                var back = $.navigatex[$.navigatex.length - 1];
                content += '<div class="treeItem trsel-back" data-id="' + back.id + '">&nbsp;<span class="fas fa-chevron-right px-5"></span>' + back.title + '</li>';
            }
            ...
            for (var ix in list) {
                ...
                // ad childs
                // check has child
                content += "<div class='row tree-row'>";
                var col = "col-11 contentfld";
                var btn_sel;
                if (item[$.trsStore[$.currentCounter].json.child].length == 0) {
                    ...
                    var a_direction = '';
                    if ($('div.trsel').hasClass('trs-rtl')){
                        a_direction = 'left';
                    }else {
                        a_direction = 'right';
                    }
                    var clsx = ' class="treeItem trsel-childer pl-5 fas fa-chevron-' + a_direction + '"';
                    btn_sel = '<span style="width: auto;" data-id="' + item.idx + '" ' + clsx + '></li>';
                    col = 'col-11 contentfld';
                }
                // li to list
                content += '<div class="col-1 arrowfld">' + btn_sel + '</div><div class="' + col + '"><li class="treeItem"' + ' data-id="' + item.idx + '"  data-value="' + item[$.trsStore[$.currentCounter].json.value] + '">' + item[$.trsStore[$.currentCounter].json.title] + '</li>';
                content += '</div></div>';
                content += '</div>';
            }
